...
Plotly figures for factor evaluation, designed to consume DataFrames produced by
factor_evaluator_plus.FactorEvaluator.* methods.

No heavy computations here—just plotting. Each function returns a Plotly Figure.
"""
...
def fig_ir_distribution_panel(ir_df: pd.DataFrame, value_col: str = 'IR',
    ...
    # Top panel: histogram + cumulative
    ...
    # Bottom panel: daily bar + cumulative
    x = ir_df.index
...
def fig_ic_ir_panel(ic_or_ir_df: pd.DataFrame, bar_col: str, cum_col: str,
                    title: str) -> go.Figure:
    """Generic 'bar + cumulative line (secondary axis)' panel."""
    fig = make_subplots(rows=1, cols=1, specs=[[{'secondary_y': True}]])
    x = ic_or_ir_df.index
    fig.add_trace(go.Bar(x=x, y=ic_or_ir_df[bar_col], name=bar_col, opacity=0.8),
                  row=1, col=1, secondary_y=False)
    fig.add_trace(go.Scatter(x=x, y=ic_or_ir_df[cum_col], name=cum_col,
                             mode='lines+markers'),
                  row=1, col=1, secondary_y=True)
...
def fig_group_navs(group_daily_returns: pd.DataFrame, title: str = "分组净值（重叠组合日频）") -> go.Figure:
    """
    Input: DataFrame where columns are group indices or names, values are *daily* returns.
           We'll cumprod to NAV (starting at 1).
    """
...
def fig_longshort_bars(ls_df: pd.DataFrame, ls_col: str = 'LS',
                       cum_line: str = 'IR_cum', title: str = "多空收益与累计指标") -> go.Figure:
    """
    Bars: LS daily returns; Line: cumulated metric (IR_cum by default).
    """
    fig = make_subplots(rows=1, cols=1, specs=[[{'secondary_y': True}]])
